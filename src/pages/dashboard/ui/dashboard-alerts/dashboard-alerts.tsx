...
import { daysUntilDeadline, getDeadlineUrgency, DEADLINE_URGENCY_COLORS } from "../../model";
...
export function DashboardAlerts({ event, totalPendingPassengers, totalPending }: DashboardAlertsProps) {
  ...
  } else if (regUrgency === "urgent" || regUrgency === "approaching") {
    alerts.push({
      icon: <Clock size={18} />,
  ...
  if (payUrgency === "expired" && totalPendingPassengers > 0) {
    alerts.push({
      icon: <AlertTriangle size={18} />,
  ...
  } else if ((payUrgency === "urgent" || payUrgency === "approaching") && totalPendingPassengers > 0) {
    alerts.push({
      icon: <Clock size={18} />,
