...
import { daysUntilDeadline, getDeadlineUrgency, getGreetingByTime, totalFromBreakdown } from "./dashboard-utils";
...
describe("getDeadlineUrgency", () => {
  it("retorna 'expired' para dias negativos", () => {
    expect(getDeadlineUrgency(-1)).toBe("expired");
    expect(getDeadlineUrgency(-10)).toBe("expired");
  });

  it("retorna 'urgent' para 0 a 3 dias", () => {
    expect(getDeadlineUrgency(0)).toBe("urgent");
    expect(getDeadlineUrgency(1)).toBe("urgent");
    expect(getDeadlineUrgency(3)).toBe("urgent");
  });

  it("retorna 'approaching' para 4 a 7 dias", () => {
    expect(getDeadlineUrgency(4)).toBe("approaching");
    expect(getDeadlineUrgency(7)).toBe("approaching");
  });

  it("retorna 'safe' para mais de 7 dias", () => {
    expect(getDeadlineUrgency(8)).toBe("safe");
    expect(getDeadlineUrgency(30)).toBe("safe");
  });
});
